...
class Koan06 extends GroovyTestCase {
	...
	void test02_CollectMethodOnLists() {
		...
		def uniqueTypes = []
		// ------------ START EDITING HERE ----------------------
		differentTypes.collect( { i ->
			uniqueTypes.contains(i.class)?:uniqueTypes << i.class
		} )

		//uniqueTypes = differentTypes.collect { it.class }.unique() // mas groovy
	...
	void test03_FileIteration() {
		...
		new File('src').eachFileRecurse(FileType.FILES) {file ->
			!file.text.contains('Lorem') ?: count++
		}
